...
public class ValoracionResource {
     ...
     * base de datos.
     *
     * @param proveedorId El ID del proveedor del cual se le agrega la valoracion.
     * @param valoracion {@link ValroacionDTO} - La valoracion que se desea guardar.
     * @return JSON {@link ValoracionDTO} - La valoracion guardada con el atributo id
     * autogenerado.
     * @throws BusinessLogicException {@link BusinessLogicExceptionMapper} -
     * Error de lógica que se genera si el texto de la valoracion excede el límite establecido (20000 caracteres).
     */
    @POST
    public ValoracionDTO createValoracion(@PathParam("proveedorId") Long proveedorId, ValoracionDTO valoracion) throws BusinessLogicException{
        LOGGER.log(Level.INFO, "ValoracionResource createValoracion: input: {0}", valoracion.toString());
        ValoracionDTO nuevaValoracionDTO = new ValoracionDTO(valoracionLogic.createValoracion(proveedorId, valoracion.toEntity()));
        LOGGER.log(Level.INFO, "ValoracionResource createValoracion: output: {0}", nuevaValoracionDTO.toString());
     ...
     * proveedor.
     *
     * @param proveedorId El ID del proveedor del cual se buscan las valoraciones.
    ...
    public List<ValoracionDTO> getValoraciones(@PathParam("proveedorId") Long proveedorId) {
        ...
        LOGGER.log(Level.INFO, "ValoracionResource getValoraciones: output: {0}", listaDTOs.toString());
        return listaDTOs;
    }
     ...
     * @param valoracion {@link ValoracionDTO} - La valroacion que se desea guardar.
     * @return JSON {@link ValoracionDTO} - La valoracion actualizada.
     * @throws BusinessLogicException {@link BusinessLogicExceptionMapper} -
     * Error de lógica que se genera cuando los caracteres de la valoracion exceden el límite permitido (20000) o el valoracionId y el ID de la valoracion no coinciden.
     * @throws WebApplicationException {@link WebApplicationExceptionMapper} -
     * Error de lógica que se genera cuando no se encuentra la valoracion.
     */
    @PUT
    ...
    public ValoracionDTO updateValoracion(@PathParam("proveedorId") Long proveedorId, @PathParam("valoracionesId") Long valoracionId, ValoracionDTO valoracion) throws BusinessLogicException {
        ...
        LOGGER.log(Level.INFO, "ValoracionResource updateValoracion: output:{0}", valoracion.toString());
        return valoracionDTO;
    }
     ...
     *
     * @param proveedorId El ID del proveedor del cual se va a eliminar la valoracion.
